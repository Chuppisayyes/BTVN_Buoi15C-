...
public class QuanLySanPham
{
    ...
    public void ThemSanPham()
    {
        ...
        switch (loai)
        {
            ...
            case 3:
                ...
                thucPham.PhiVanChuyen = phiVanChuyen;
                sanPham = thucPham;
                break;
    ...
    public void DocDuLieuTuFile()
    {
        if (File.Exists(filePath))
        {
            ...
            var tempList = JsonConvert.DeserializeObject<List<dynamic>>(json);
            danhSachSanPham.Clear();

            foreach (var item in tempList)
            {
                switch ((string)item.LoaiSanPham)
                {
                    case "DienTu":
                        danhSachSanPham.Add(JsonConvert.DeserializeObject<DienTu>(item.ToString()));
                        break;
                    case "ThoiTrang":
                        danhSachSanPham.Add(JsonConvert.DeserializeObject<ThoiTrang>(item.ToString()));
                        break;
                    case "ThucPham":
                        danhSachSanPham.Add(JsonConvert.DeserializeObject<ThucPham>(item.ToString()));
                        break;
                    default:
