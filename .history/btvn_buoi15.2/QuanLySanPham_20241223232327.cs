...
public class QuanLySanPham
{
    ...
    public void LuuDuLieuVaoFile()
    {
        try
        {
            ...
            // Lưu từng loại sản phẩm vào các file riêng biệt
        ...
        if (File.Exists(filePath))
        {
            try
            {
                ...
                if (tempList != null)
                {
                    ...
                    {
                        if (item != null && !string.IsNullOrEmpty(item.LoaiSanPham))
                        {
                            if (item.LoaiSanPham == "DienTu")
                            {
                                var dienTu = JsonConvert.DeserializeObject<DienTu>(JsonConvert.SerializeObject(item));
                                danhSachSanPham.Add(dienTu);
                            }
                            else if (item.LoaiSanPham == "ThoiTrang")
                            {
                                var thoiTrang = JsonConvert.DeserializeObject<ThoiTrang>(JsonConvert.SerializeObject(item));
                                danhSachSanPham.Add(thoiTrang);
                            }
                            else if (item.LoaiSanPham == "ThucPham")
                            {
                                var thucPham = JsonConvert.DeserializeObject<ThucPham>(JsonConvert.SerializeObject(item));
                                danhSachSanPham.Add(thucPham);
                            }
                            else
                            {
                                Console.WriteLine("Không xác định được loại sản phẩm: " + item.LoaiSanPham);
                            }
                        }
                        else
                        {
                            Console.WriteLine("Sản phẩm không hợp lệ hoặc thiếu thông tin loại sản phẩm.");
                        }
                    }
                }
                else
                {
                    Console.WriteLine("Dữ liệu trong file không hợp lệ.");
                }
            }
            catch (Exception ex)
            {
