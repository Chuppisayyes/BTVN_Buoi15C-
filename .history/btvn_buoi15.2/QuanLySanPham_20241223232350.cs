...
public class QuanLySanPham
{
    ...
    public void LuuDuLieuVaoFile()
    {
        try
        {
            var dienTuList = danhSachSanPham.OfType<DienTu>().ToList();
            var thoiTrangList = danhSachSanPham.OfType<ThoiTrang>().ToList();
            var thucPhamList = danhSachSanPham.OfType<ThucPham>().ToList();
            File.WriteAllText("dienTu.json", JsonConvert.SerializeObject(dienTuList, Formatting.Indented));
            File.WriteAllText("thoiTrang.json", JsonConvert.SerializeObject(thoiTrangList, Formatting.Indented));
            File.WriteAllText("thucPham.json", JsonConvert.SerializeObject(thucPhamList, Formatting.Indented));

            Console.WriteLine("Dữ liệu đã được lưu vào các file.");
        }
        catch (Exception ex)
        {
    ...
    public void DocDuLieuTuFile()
    {
        try
        {
            // Đọc và deserializing từ các file
            var dienTuList = File.Exists("dienTu.json") ? JsonConvert.DeserializeObject<List<DienTu>>(File.ReadAllText("dienTu.json")) : new List<DienTu>();
            var thoiTrangList = File.Exists("thoiTrang.json") ? JsonConvert.DeserializeObject<List<ThoiTrang>>(File.ReadAllText("thoiTrang.json")) : new List<ThoiTrang>();
            var thucPhamList = File.Exists("thucPham.json") ? JsonConvert.DeserializeObject<List<ThucPham>>(File.ReadAllText("thucPham.json")) : new List<ThucPham>();

            // Kết hợp tất cả sản phẩm vào danh sách chung
            danhSachSanPham.Clear();
            danhSachSanPham.AddRange(dienTuList);
            danhSachSanPham.AddRange(thoiTrangList);
            danhSachSanPham.AddRange(thucPhamList);

            Console.WriteLine("Đọc dữ liệu từ các file thành công.");
        }
        catch (Exception ex)
        {
