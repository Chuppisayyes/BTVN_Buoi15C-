...
public class QuanLySanPham
{
    ...
    public void ThemSanPham()
    {
        ...
        double giaGocInput = double.Parse(Console.ReadLine());

        switch (loai)
        {
            case 1:
                sanPham = new DienTu { aSanPham = maSP, tenSanPham = tenSP, giaGoc = giaGocInput };
                Console.Write("Nhập thuế bảo hành: ");
                ((DienTu)sanPham).BaoHanh = double.Parse(Console.ReadLine());
                break;
            case 2:
                sanPham = new ThoiTrang { maSanPham = maSP, tenSanPham = tenSP, giaGoc = giaGocInput };
                Console.Write("Nhập giảm giá: ");
                ((ThoiTrang)sanPham).GiamGia = double.Parse(Console.ReadLine());
                break;
            case 3:
                sanPham = new ThucPham { maSanPham = maSP, tenSanPham = tenSP, giaGoc = giaGocInput };
